...
window.extraProducts = extraProducts;

function getStoredCart() {
    try {
        return JSON.parse(localStorage.getItem('crystalCart') || '[]');
    } catch (error) {
        return [];
    }
}
...
function addProductToCart(product) {
    ...
    const existing = storedCart.find(item => item.id === product.id && item.category === product.category);
    ...
    if (notification) {
        notification.querySelector('#notificationText').textContent = `تم إضافة "${product.name}" إلى السلة`;
    ...
    grid.innerHTML = extraProducts.map(product => `
        <article class="product-card">
            ...
            <div class="product-info">
                ...
                <button class="add-to-cart" data-product-id="${product.id}"><i class="fas fa-shopping-bag"></i> أضف إلى السلة</button>
            </div>
        </article>
    `).join('');
